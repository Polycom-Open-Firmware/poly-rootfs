...
Slot-aware busybox initramfs. Boots first, picks the active A/B slot from the
kernel command line, mounts the matching ext4 root, and `switch_root`s into it.
...
Or run the top-level `./build.sh`, which builds the rootfs first and then
calls this script with `BUSYBOX=work/rootfs/usr/bin/busybox`.
